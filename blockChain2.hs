...
mejorSegunSaldo criterio (primerUsuario:restoUsuarios) = foldr (compararSegun criterio billetera) primerUsuario restoUsuarios

encontrarMejorUsuarioSegun criterio bloque listaDeUsuarios = fromJust (find (compararUsuariosPorPorNombre (mejorSegunSaldo criterio (aplicarBloqueAMuchos bloque listaDeUsuarios))) listaDeUsuarios)

elUsuarioMasRickyFord = encontrarMejorUsuarioSegun max
elUsuarioMasPobre = encontrarMejorUsuarioSegun min

usuariosQueLleganANCreditos bloque unMonto = filter (saldoAlMenosNMonedas unMonto.aplicarBloque bloque)

aplicarBlockChain blockChain unUsuario = foldr aplicarBloque unUsuario blockChain

peorBloqueParaUnUsuario (bloque:restoBloques) unUsuario = foldr (compararSegun min (billetera.flip aplicarBloque unUsuario)) bloque restoBloques

aplicarPrimerosNBloquesAUsuario n blockChain = aplicarBlockChain (take n blockChain)

aplicarBlockChainAMuchos unBlockchain = map (aplicarBlockChain unBlockchain)


bloquesParaLLegarNMonedas n (bloque:restoBloques) unUsuario
  | billetera unUsuario < n = bloque : bloquesParaLLegarNMonedas n restoBloques (aplicarBloque bloque unUsuario)
  | otherwise = []
